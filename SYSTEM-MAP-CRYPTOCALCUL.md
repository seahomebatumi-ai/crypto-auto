...
**Revision 2026-09-16-b.** Baseline: one report-only TZ, one methodology revision and one
contract version.
**TZ-46** probed class 2 for the two coins TZ-45 left owed one, with TZ-45's own instrument
and one request each: ETH answers on the Ethereum Foundation's blog feed
(`blog.ethereum.org`, `pubDate` on every item of its one page) and ADA on the
`IntersectMBO/cardano-node` release list (`api.github.com`, `published_at` on five of five),
both `primary-dated`, and the stop rule withheld the third candidate — go-ethereum's releases
are unmeasured, not negative (§10). Report
`CryptoReports/TZ-46-eth-ada-release-channels-report.md`, accepted.
**There is no implementation commit to name**: the class is report-only (contract §8), the
report is the whole of the change, and every figure in it was measured on `e04eb3f`, the
Boss's upload of the specification. The reading reaches the engine as an Architect edit and
not as a TZ — `ANALYST-INSTRUCTIONS.md` revision `2026-09-16-c` names 29 channels in
methodology §6a, ETH and ADA carrying two lanes each, and its §11 schema gains `c2` so the
second lane has somewhere to be stored.
...
diff, and no workflow moved. TZ-46's session re-read the four rows of the file table below and
the five bench figures carried further down on `e04eb3f`, and all nine sit at the values this
block records — so they stand on a reading rather than on inheritance, and the three benches
TZ-44 moved have now been re-measured by two sessions other than the one that moved them.

**The Project mirrors were MATCHED against that tree, not assumed.** Before this revision's
edits, this map, `EXECUTOR-INSTRUCTIONS.md` and `ANALYST-INSTRUCTIONS.md` each reproduced the
report's line count and MD5 on the Architect's own copy, and so did the `## 6` + `6a` digest
the methodology keys lane staleness to (`a121c493be25b8a7a25c505a600b1a95`) — so mirror and
repository were the same bytes at `e04eb3f`, and the three files this revision publishes are
edits of that reading. **That is a computation and never an inheritance:** a mirror whose
version line is behind the repository is not a weaker source but a wrong one, and nothing but
a hash of both says which it is.
...
carried no code and moved no file. **Every dispatch figure below therefore predates TZ-44**
and was taken on the cache-wide window it retired: the next dispatch is the first reading of
`--verify` under per-symbol comparability. **The run was RETAINED**: `attrib.txt` is in the artifact, which is the
reading inv. 71's first half asked for and the first time an `--attrib` figure has existed
outside a job log. Its second half is still unread — `--verify` exited 0 on this run, so no
step ever stood in the shadow the conditioning repair exists to lift (§10).

**The finding is that the finding is gone.** `--verify` compared 30 coins and returned
**`unexplained` 0, `coverage` 0**: 25 spot coins `clean`, five perpetuals `venue-basis`, which
is reference and not a failure. The worst spot cells are levels ≤ 0.73 % against a 2 % bar,
`volatility` 2.30 % against 10 %, `vol7` 16.2 % against 25 %, and returns ≤ 1.18 pp against
1.50 / 2.00 / 3.00. The 35 `unexplained` cells of 09.09 — one quantity at four horizons, up
to +5.15 pp, positive on 24 of 25 — **did not reproduce**, `--target` therefore ran with
**30 coins in the arms and none excluded**, and the twelve-coin universe every gated figure
was taken on is retired as a state (§7, §3.10a). **What changed between the two readings is
NOT established here** and the candidates are not separable by one run: the transport repair
(TZ-39 removed the path where a leg that could not be READ still won a comparison as clean),
the sign and size of the gap (+0.8 h then, −0.2 h now), and a different day. The next
dispatch either reproduces it or does not, and `--attrib` now stands ready to attribute it if
it does.
...
**The PREVIOUS dispatch — the one this map waited on since TZ-36 — and what it answered.**
Taken
...
203, 266, 323, 373, 467, 475 and now 487.** §3.10 and §10 both carried the figure TZ-30 measured, and neither
...
**The revisions before this one, in one line each.** `2026-09-16-a` recorded TZ-45 — one
...
Contract **v23** — 864 lines, MD5 `02abb1969626d2af150a0d1f6e02f2a7`. **Three versions moved
since `2026-09-14-a`, and none touches a production file.** v21 — 831 lines, `5b125d9a39bc4ccd4e935d01ed1aa1e8`,
...
`§0`, as TZ-28 through TZ-44 all did: 2544 → 2768 → 3216 → 3240 → 3724 → 3881 → 4110 → 4258 →
4555 → 4572 → 5009 → 5008 → **5102 lines**, `ba633202f43845ba0fdafbc1b92d9c04`. **The one fall in
that ladder is a repair**: TZ-41 replaced two lines of local-clock arithmetic with one call to
`calendar.timegm`. Four more benches carry figures for the same reason:
`bench/backtest_guard_bench.py` **2503 lines**,
`bfc984b1d22ec1ad89cf536a1a47c529`, `bench/verify_bench.py` **540 lines**,
`28eb1949f21d0afadb062303108f7101`, `journal/write.js` **849 lines**,
`19722fb53d75b6d25a8f957f74f97422`, and `bench/journal_bench.js` **1177 lines**,
`993271f44995c8ae21c54935a3f80adf`.
...
`2026-09-14-a`: neither TZ-45 nor TZ-46 moved a file any step reads. That revision is where TZ-44 moved TWO
...
**The hosted gate WAS read this time, by the session that produced it**: two `Bench gate` runs
on the implementation commit `99e4b0a`, `34786159289` (push) and `34786168232`
(pull_request), both `success` on every job step, read with `gh run view`. **What nobody has
read is the runner's own per-step COUNTS** — a conclusion is a measurement and a count is a
different one, and neither substitutes for the other (inv. 22, 43). **Step 5's own count
has never been recorded in this map**: it is the residual that makes the recorded total add up
...
hand-built dates (TZ-37) · and `_cell_comparable`'s decision, section K (TZ-44). `requests` is stubbed and every assertion about a host that
...
71. **A measurement that is not RETAINED was not taken, and a step that is not CONDITIONED
    ...
    13.09.2026** — `attrib.txt` came back inside the artifact — **and conditioning was not**:
    that run's `--verify` exited 0, so nothing stood in the shadow (§10).
...
|---|---|---|
| Class-A coin channels had never been measured | **measured 15.09.2026T22:35–22:39Z by TZ-45 — 27 of 30 answer with dated records** | a run recording one of the channels refused or landing on another host, which is the re-measurement. Methodology §6a (`ANALYST-INSTRUCTIONS.md` `2026-09-16-c`) names them with the one command form each was measured with: fifteen Discourse forums read at `/latest.json`, eleven release lists on `api.github.com`, one on `gitlab.com`, RENDER's feed on `rendernetwork.medium.com` and the Ethereum Foundation's on `blog.ethereum.org` — **29 rows for 27 coins**, ETH and ADA carrying two lanes each since TZ-46. Two forums land elsewhere — `forum.sky.money` on `forum.skyeco.com`, `forum.algorand.org` on `forum.algorand.co` — and the lane records the host that answered. **What was measured is a dated STREAM and not an event date:** each date field says when a record was made, and whether a channel carries class-A dates in its content was deliberately left unread. Two residuals are properties of the channels rather than defects: each read is one page, so a busy forum can outrun a weekly read and the methodology records the unread stretch instead of an absence; and the eleven GitHub lanes share one unauthenticated quota, 60 requests an hour as measured, so a refusal there is one refusal of all eleven. **A coin with no row costs no request**, so HYPE's unestablished candidate on that host is not a twelfth lane — TZ-46's own risk list counted it as one and the methodology says so in one clause |
...
| The return family disagrees with production and the levels do not | **did not reproduce — measured 13.09.2026 on 30 coins, `unexplained` 0** | the next dispatch, and nothing before it. The instrument this row asked for exists and ran; the phenomenon did not. **Both readings stand and neither explains the other**: 09.09 gave 35 cells on eighteen coins, one quantity at four horizons, up to +5.15 pp, positive on 24 of 25; 13.09 gave zero failing cells with the same fields, the same thresholds and the same archive source. Three candidates separate the runs and one dispatch cannot rank them — TZ-39 closed the path where a leg that could not be READ still won a comparison as clean (a series short by the pages that did not parse is the classic silent truncation, inv. 63, 64); the gap ran +0.8 h then and −0.2 h now, and one hour of common market move lands on every coin with the same sign; and four days passed. **The consequence that DOES follow is a state change**: `--target` ran on 30 coins with none excluded, so every gated figure is now taken on the full universe and the twelve-coin description is retired (§7, §3.10a, §3.10b) |
...
| The retry ladder has still never been seen to fire | watched — **measured 13.09.2026: 0.0 s of 600 on both passes** | the first dispatch that prints a non-zero figure. TZ-39's line now prints on every pass — `связь исчерпана на 0 монет(ах):  — ретраи потратили 0.0 с из 600` on `--fetch` and on `--fetch-funding`, 31 of 31 coins cached, nothing lost — so the budget is visible and the reading is a zero. **A visible zero is worth more than the invisible number it replaced and is still not evidence that `_http`'s ladder has ever executed**: the same output follows from a run that needed no retry and from one whose retries the code never reaches. `HTTP_BUDGET_S` stays where TZ-38 derived it until a run spends some of it |
| The dispatch cache is frozen at 05.09 and every run refetches the universe | **open — measured 13.09.2026** | any TZ opening `backtest_bench.yml`. `actions/cache@v4` is keyed `bench-${{ inputs.source }}-${{ inputs.years }}y-v4`, and a cache is saved only when its key MISSES; the key has hit since the run that created it, so `bench/cache` is restored at documents ending **2026-09-05T20** on every dispatch and the run's own fetch is thrown away at the end of the job. Two consequences, both visible in this artifact: every coin prints «площадка не записана — перекачка» and pays a full history download, which is TZ-34's venue observation being re-derived and re-discarded on every run; and the map's reading of 09.09 — «every cached document was refetched because none carried an observed venue» — was the same freeze rather than a one-off. **The freeze is not a correctness defect**: each run measures on the archive it just fetched. It costs a full refetch per dispatch and makes a permanent line look like a finding |
| Inv. 71's conditioning half has no reading | **open** | the first dispatch on which `--verify` exits non-zero. `attrib.txt` and `regimes.txt` are both in the 13.09 artifact, but that run's `--verify` was GREEN, so neither step ever stood behind a failure and the `if: ${{ !cancelled() }}` on them was never load-bearing. **A repair whose evidence is the absence of the failure it prevents has no reading of its own** (inv. 22, 43) — the same standing TZ-38's transport carried for two revisions. Retention is read; conditioning is reserved |
