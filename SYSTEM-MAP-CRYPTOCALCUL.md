...
**Revision 2026-09-26-a.** One methodology revision, forced by the audit of the engine's run of
25.09.2026, 22:22 Tbilisi, and the measurements it rests on; **no production file moves**, so every
row of the file table below stands. `ANALYST-INSTRUCTIONS.md` `2026-09-30-c` gives the engine an
...
|---|---|
| revision | `**Revision 2026-09-26-a.**` |
...
| Unlock dates have no primary lane | **discovery half closed by `ANALYST-INSTRUCTIONS.md` `2026-09-30-c`; primary half open — measured 25.09.2026 from the Architect's session** | the first analysis run under `-30-c`, whose log records the unlock lane's landing host and status per coin: that is the reading on the Executor's machine, and a refusal there re-opens this row whole. **What the lane gives:** `tokenomist.ai/<id>`, `<id>` the CoinGecko id of `TOKENS`, answered 200 for all thirty; the site carries 22, and its served HTML states for seven the next cliff in one sentence — tokens, value, date, recipient and released supply — and for fifteen that the coin is fully unlocked; the eight it does not carry — XRP, GRAM, TRX, BCH, SKY, HBAR, XLM, XMR — answered with neither. The methodology treats the sentence as `reported`: it closes a long, prints `НЕ ПОДТВЕРЖДЕНО`, and backs no level. **What stays open is the primary:** a published date, `XXX до ДД.ММ` and a short resting on a cliff still need the protocol's own schedule, read per material cliff as a lookup, and the one contract-state reading taken — TZ-45's, on ONDO — dates nothing, because a cliff lives in a vesting contract whose address is the protocol's own. **This is the engine's rule, not the calculator's:** §8's closure of automating unlocks stands for production |
...
methodology `2026-09-30-b` one market read is in the method, and it is not a price:** item 90's
move for the five declared perpetuals, from `fapi.binance.com`'s daily klines, which stands
behind no level (§10).
...
`reported` — a class that closes a side and opens none — and its first read is this machine's
measurement of the page. `cryptorank.io` and the unlock-events page stay closed (§10).
