...
2. **Coverage is 25 of 28 by construction.** The three `fut:true` assets
   ...
   field that says «degraded» every single day says nothing at all.
   **Fixed 22.08, TZ-07 (PR #7).**

   <!-- EDIT-MARKER 2026-08-22-COVERAGE-SEMANTICS -->

   The venue test now short-circuits **ahead of all five** classifier branches
   (absent row → dead market → no bot row → error flag → no metrics), so a
   `fut:true` asset can never reach one that increments `hardSkip`. The reason
   string still records what was *observed* — inv. 37 wants the gap measured,
   not assumed — in exactly three forms:
   ...
   The third case contradicts the §3.14 declaration and must not pass silently:
   it additionally pushes `fut:true asset trading on spot: <SYM>` into
   `run.note`, one note per symbol, so the anomaly reaches `runs.jsonl`. A
   relisting is a `tokens[]` decision for the Architect, never the journal's.
   Non-`fut` classification is untouched in ordering, strings and counters: a
   dead **spot** pair still hard-skips and still degrades `status`, which is the
   control that proves the degradation signal was removed only where it was
   false. Healthy day, replayed through production `snapshot()` on the recorded
   21.08 inputs: `cov 25 · skip 3 · hardSkip 0 · status ok · note null`. The
   records already written as `partial` stay `partial` — inv. 38 makes them
   immutable, and they are honest records of what the classifier said that day.
...
41. **Объявленная площадка читается ДО лестницы деградации.** `fut:true` — это ОБЪЯВЛЕНИЕ (§3.14), а не наблюдение, поэтому пропуск по такому активу есть ОБЪЯВЛЕННОЕ покрытие в любой своей форме и `hardSkip` не поднимает никогда. Обратный порядок уже стоил поля `status`: зеркало отдавало делистнутую строку `XMRUSDT`/`LITUSDT`, классификатор читал её как `dead market`, и здоровая система рапортовала `partial` каждые сутки. Причина при этом ИЗМЕРЯЕТСЯ тремя разными строками (нет пары / строка мертва / строка ЖИВА), а живая спот-пара у `fut:true` актива обязана дойти до `run.note` — противоречие объявлению не имеет права пройти тихо. Правило шире журнала: любой будущий потребитель `tokens[]` обязан спрашивать объявление, а не хост.
42. **Стенд обязан исполнять продакшн с ТЕМ ЖЕ внешним входом, что и продакшн.** `verify_board.js`, `board2_bench.js` и `prot_bench.js` восемь дней гоняли доску с пустым `CATALYSTS`, потому что в песочнице нет `XMLHttpRequest`: загрузчик молча падал, реестр оставался `{}`, и стенды воспроизводили конфигурацию, которой нет ни у Босса, ни на GitHub Pages. Пока все записи `disputed`, разницы нет — первая же `confirmed` делает её решающей. Поэтому: реестр читается из чекаута ТЕМ ЖЕ загрузчиком, что и в продакшне (инв. 21, никакой второй реализации), инъекция идёт ПОСЛЕ `vm.runInContext` (иначе продакшн-строка `var CATALYSTS = {}` затирает её), а отсутствие или порча файла роняет стенд НЕНУЛЕВЫМ кодом — отката к пустому реестру нет. Родня инв. 22: стенд, проверяющий не ту систему, — это стенд, не проверяющий ничего.
43. **Счёт проверок обязан БЫТЬ счётом.** Число, которое стенд печатает как «проверок», используется как доказательство объёма контроля и как вход инв. 22 — значит оно обязано считать сверки, а не оцениваться произведением. В `direction_bench.py` три блока возвращают выдуманные величины: `--props` печатает `checks × 8`, `--fixtures` — `строк × 4`, `--display` — `списков × торгуемых + len(tier)`, то есть произведение двух не связанных величин. Такой счёт нельзя ни сверить, ни разложить: он двигается от вещей, которые не являются сверками, и делает страж «сверил ноль» декоративным. Правило: счётчик инкрементируется в точке сравнения, итог ворот складывается из этих счётчиков, и любое расхождение объясняется по слагаемым. **Нарушение открыто аудитом 22.08 и закрывается ТЗ-08; до его слияния итог ворот 1 308 873 завышен на неизвестную величину и как доказательство объёма не годится.**
...
## 9. Журнал миграций
- 2026-08-22 (2): **ТЗ-07 в проде — PR #7. Контроли восстановлены; торговая математика не тронута ни на символ.**

  <!-- EDIT-MARKER 2026-08-22-GATE-COMPOSITION -->

  `index.html`, `main.py` и `catalysts.json` байт-в-байт равны состоянию до правки — это проверено диффом, а не заявлено. Четыре независимых участка. **(A) Семантика покрытия журнала** переведена на объявление площадки: новый **инвариант 41**, §3.13 п. 2 переписан, `status` перестал быть постоянным `partial`. **(B) Паритет реестра в стендах доски**: новый **инвариант 42**; `verify_board.js`, `board2_bench.js`, `prot_bench.js` читают `catalysts.json` из чекаута продакшн-загрузчиком, ни одно старое число не сдвинулось (108/129/167 → 109/130/168, ровно +1 на новую проверку «реестр загружен и не пуст»), значит пустой реестр ничего не маскировал. **(C) Контракт отображения вернулся под исполняющийся контроль**: `display_bench.py`, `render_bench.py`, `direction_bench.py --display` встали в ворота; их ожидания были устаревшими (ред. 19.08 (3)), продакшн при сверке оказался прав — низший тир `Фон`, номер после слова тира перед em dash, номер у КАЖДОЙ показанной строки со счётом. Инв. 30 не ослаблен: гарантию «одна монета — одна сторона» несёт `action` и проверяет `--props` (счётчик `both`), а не номер. **(D) Ворота исполняются на ветке**: `bench.yml` триггерится на `push` в `main` и `claude/**`; `pull_request` и `paths-ignore` не тронуты. Доказано негативным контролем на раннере: намеренно подсаженный провал `display_bench.py` уронил прогон #29 на ветке БЕЗ pull request, revert вернул зелёное. До этой правки 1 199 724 проверки ТЗ-06 не исполнились ни на одном раннере до слияния — это и есть цена, которую закрывает пункт (D).

  Состав ворот: 11 шагов. Аудит 22.08 добавил к этому две поправки к отчёту исполнителя, обе проверены исполнением: страж «сверил ноль» уже стоял в `fresh_bench.js` и в `direction_bench.py` (в `main()`, то есть во ВСЕХ блоках, а не только в `--display`) — реально без стража остались ровно четыре стенда ворот (`verify_board.js`, `board2_bench.js`, `prot_bench.js`, `verify_bench.py`); и риск «`confirmed`-запись сдвинет фикстуры стендов доски» ИЗМЕРЕН и не подтвердился — при двух записях, переведённых в `confirmed` с датами внутри окна, все три дали те же 109/130/168. Зато `catalyst_bench.js` на такой правке падает штатно: `confirmed` без кворума источников не проходит (инв. 39), то есть дисциплина наполнения реестра уже под исполняющимся контролем. Открытый долг вынесен в **инвариант 43** (счёт проверок обязан быть счётом) и в ТЗ-08.
...
- **Слой контекста: как рыночная разведка попадает в калькулятор (архитектура принята 20.08, код НЕ написан).**
  ...
  **(0) Достоверность свежести.** Бот обязан возвращать ненулевой код, когда запись не состоялась; `generated_at` обязан быть виден в логе прогона; ночная пауза обязана отличаться от сбоя. Без этого дыра в выборке неотличима от «событий не было», а выборка с необъяснимыми дырами не выдерживает ни одного статистического утверждения. ТЗ-04. — **ЗАКРЫТ 21.08, PR #4.**
  ...
  ~~**(2a) Восстановить контроли — ТЗ-07.**~~ — **ЗАКРЫТ 22.08, PR #7,** по всем четырём названным дефектам; см. запись 22.08 (2) в §9. Остаток вынесен в (2c). Вынос реестра в файл и первый живой прогон журнала обнажили четыре места, где проверка выглядит зелёной, ничего не проверив (инв. 22/25/29/37): `status` журнала деградирован каждый день по классификатору пропусков; три доских стенда исполняют доску с пустым реестром; ворота `bench.yml` не запускаются на ветке без PR; контракт отображения (инв. 33–34) вообще не имеет исполняющегося стенда. Это ставится ПЕРЕД наполнением реестра источниками: первая же `confirmed`-запись меняет вердикт, и менять вердикт при неработающих контролях — ровно та последовательность, которую §10 запретил себе в пункте (2).
  **(2c) Остаток контролей — ТЗ-08, следующее.** Два дефекта, оба найдены аудитом 22.08, а не отчётом: четыре стенда ворот (`verify_board.js`, `board2_bench.js`, `prot_bench.js`, `verify_bench.py`) до сих пор выходят нулём, сверив ноль (инв. 22); и счёт проверок в трёх блоках `direction_bench.py` — произведение, а не счёт (инв. 43), из-за чего итог ворот как доказательство объёма не годится. Ни то, ни другое не меняет ни одной формулы; оба закрывают ровно тот класс, ради которого написано ТЗ-07.
  **(2b) Источники и `confirmed`** — проверить три существующие записи, заполнить `src`, перевести прошедшие кворум в `confirmed`, устранить известные расхождения данных (дата голосования ZEC; отсутствующий разлок ENA). Это аналитическая работа с веб-проверкой, отдельным нарядом. Идёт ПАРАЛЛЕЛЬНО (2c), а не после: измерено 22.08 — стенды доски к `confirmed`-записи безразличны, а `catalyst_bench.js` уже enforce'ит кворум источников, то есть контроль наполнения реестра стоит и работает.
  **(3)** Только когда выборка есть — бэктест одного фактора из класса 2. Пункт (3) не открывать, пока не закрыт (2c).
