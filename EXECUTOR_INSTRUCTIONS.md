...
**Version 3.** Permanent operating contract for the Claude Code Executor. Read this
...
Supersedes Version 2. Version 3 adds the report-delivery rule (§8), the closing
message to the Boss (§11) and the file lifecycle table (§13).
...
11. Post the closing message to the Boss in Russian (§11).
...
```
Босс, TZ-NN выполнено. Отчёт готов: `CryptoReports/TZ-NN-<name>-report.md` —
он уже в ветке `main`. Заберите его по этому пути и передайте Архитектору.
Pull request #N открыт, CI зелёный. Merge — только после вердикта Архитектора.
```
...
| Executor contract | `EXECUTOR-INSTRUCTIONS.md` | Architect | session attachment | one copy, replaced in place |
| Specifications | `CryptoTZ/TZ-NN-*.md` | Architect | session attachment | **permanent, immutable** |
